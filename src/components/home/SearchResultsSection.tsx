  ...
  return (
    <section className="mt-5">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {results.map((item) => (
          <Link key={item.id} href={`/word/${encodeURIComponent(item.word)}`}>
            <motion.div
              whileHover={{ y: -4 }}
            ...
            >
              {/* Hover Glow */}
              <div className="absolute inset-0 bg-linear-to-r from-blue-50/0 via-blue-50/40 to-violet-50/0 opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
